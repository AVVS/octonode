...
(function() {
  ...
  Repo = (function() {
    ...
    Repo.prototype.events = function() {
      var cb, page, params, _i;
      params = 2 <= arguments.length ? __slice.call(arguments, 0, _i = arguments.length - 1) : (_i = 0, []), cb = arguments[_i++];
      page = params[0] || 1;
      return this.client.get("/repos/" + this.name + "/events", page, function(err, s, b, headers) {
        if (err) {
          return cb(err);
        }
        if (s !== 200) {
          return cb(new Error("Repo activity error"));
        } else {
          return cb(null, b, headers);
        }
      });
